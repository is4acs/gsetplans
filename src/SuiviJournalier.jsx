...
import { Upload, Calendar, TrendingUp, TrendingDown, Target, AlertCircle, BarChart3, Download, Trash2, Euro, Users, CheckCircle, XCircle, Loader2 } from 'lucide-react';
import { BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer, LineChart, Line, Legend } from 'recharts';
import * as XLSX from 'xlsx';
import { getDailyTracking, insertDailyTracking, createDailyImport, getDailyImports, deleteDailyImport } from './lib/supabase';
...
function parseSuiviJournalier(worksheet, type) {
  ...
  for (let i = headerRow + 1; i < json.length; i++) {
    ...
    data.push({
      ...
      planifies: parseInt(row[4]) || 0,
      realises: parseInt(row[5]) || 0,
      ok: parseInt(row[6]) || 0,
      nok: parseInt(row[7]) || 0,
      reportes: parseInt(row[8]) || 0,
      taux_reussite: parseFloat(row[9]) || 0,
      taux_echec: parseFloat(row[10]) || 0,
      taux_report: parseFloat(row[11]) || 0,
      taux_cloture: parseFloat(row[12]) || 0,
    });
  }
...
export default function SuiviJournalier({ theme = 'light', userRole = 'tech', currentTechName = null }) {
  ...
  const [importStatus, setImportStatus] = useState(null); // {success: bool, message: string}
  ...
  const loadData = useCallback(async () => {
    setLoading(true);
    try {
      const [trackingData, importsData] = await Promise.all([
        getDailyTracking(),
        getDailyImports()
      ]);
      setSuiviData(trackingData);
  ...
  // Import fichier SUIVI vers Supabase
  const handleFileUpload = useCallback(async (e) => {
    ...
    reader.onload = async (evt) => {
      try {
        ...
        if (allData.length === 0) {
          setImportStatus({ success: false, message: 'Aucune donnée trouvée dans les feuilles SUIVI JOURNALIER CANAL/ORANGE' });
          return;
        }
        
        // Ajouter la période à chaque enregistrement
        const periode = `${new Date().toLocaleDateString('fr-FR')} - ${file.name}`;
        const dataWithPeriode = allData.map(d => ({ ...d, periode }));
        
        // Sauvegarder dans Supabase
        await insertDailyTracking(dataWithPeriode);
        ...
        await createDailyImport({
          ...
          date_debut: new Date(Math.min(...dates)).toISOString().split('T')[0],
          date_fin: new Date(Math.max(...dates)).toISOString().split('T')[0],
          periode
        });
        
        setImportStatus({ success: true, message: `${allData.length} enregistrements importés avec succès !` });
        ...
        setImportStatus({ success: false, message: `Erreur: ${error.message}` });
      } finally {
        setImporting(false);
      }
    };
  ...
  // Supprimer un import
  const handleDeleteImport = useCallback(async (imp) => {
    if (!confirm(`Supprimer l'import "${imp.filename}" et toutes ses données ?`)) return;
    try {
  ...
  const chartData = useMemo(() => {
    ...
    return Object.values(grouped)
      ...
      .map(d => ({
        ...
        // Mapper les noms de colonnes Supabase vers le format affichage
        tauxReussite: d.taux_reussite || d.tauxReussite || 0
      }));
  }, [filteredData]);
  ...
  return (
    <div className={`min-h-screen p-6 ${t.bg}`}>
      <div className="max-w-7xl mx-auto space-y-6">
          ...
          <div className="flex gap-3 flex-wrap items-center">
            {userRole === 'admin' && (
              <label className="px-4 py-2 bg-emerald-500 text-white rounded-lg hover:bg-emerald-600 transition-colors cursor-pointer flex items-center gap-2">
                ...
                {importing ? <><Loader2 className="w-4 h-4 animate-spin" /> Import...</> : 'Importer SUIVI D3'}
                <input
                  type="file"
                  accept=".xlsx,.xls"
        ...
        {importStatus && (
          <div className={`p-4 rounded-lg flex items-center gap-3 ${importStatus.success ? 'bg-emerald-50 dark:bg-emerald-900/20 text-emerald-700 dark:text-emerald-400' : 'bg-red-50 dark:bg-red-900/20 text-red-700 dark:text-red-400'}`}>
            {importStatus.success ? <CheckCircle className="w-5 h-5" /> : <XCircle className="w-5 h-5" />}
            {importStatus.message}
          </div>
        )}

        {/* Historique des imports (admin only) */}
        {userRole === 'admin' && imports.length > 0 && (
          <div className={`${t.card} rounded-lg border ${t.border} p-4`}>
            <h3 className={`font-semibold ${t.text} mb-3`}>Historique des imports</h3>
            <div className="space-y-2">
              {imports.slice(0, 5).map(imp => (
                <div key={imp.id} className={`flex items-center justify-between p-3 rounded-lg ${t.bgTertiary}`}>
                  <div>
                    <span className={`font-medium ${t.text}`}>{imp.filename}</span>
                    <span className={`ml-3 text-sm ${t.textMuted}`}>{imp.total_records} enregistrements • {imp.date_debut} → {imp.date_fin}</span>
                  </div>
                  <button onClick={() => handleDeleteImport(imp)} className="p-2 text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg">
                      ...
                      <tr key={idx} className="hover:bg-gray-50 dark:hover:bg-gray-800/50">
                        ...
                        <td className="px-4 py-3 text-center">
                          <span className={`font-semibold ${
                            (item.taux_reussite || 0) >= 0.7 ? 'text-green-600 dark:text-green-400' : 
                            (item.taux_reussite || 0) >= 0.5 ? 'text-yellow-600 dark:text-yellow-400' : 
                            'text-red-600 dark:text-red-400'
                          }`}>
                            {((item.taux_reussite || 0) * 100).toFixed(1)}%
                          </span>
                        </td>
